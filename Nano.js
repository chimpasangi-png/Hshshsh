...
async function getUser(chatId, username = "NoUsername", name = "User") {
  // ✅ Fixed duplicate key error with atomic upsert
  const user = await User.findOneAndUpdate(
    { chatId },
    { $setOnInsert: { username, name, demoUsed: false } },     { new: true, upsert: true }
  );                                                         return user;
}
...
function getBalance(amount) {
  ...
  if (amount >= 2) return "$20";                             return null;
}                                                          
// ---------------- START MENU ----------------
...
{                                                                chat_id: chatId,                                           message_id: query.message.message_id,
  ...
  } else if (data === "support") {                             bot.editMessageText(`📩 Support Center
                                                           Choose option:`,
    {                                                            chat_id: chatId,
...
Type your question or issue now:`);
    ...
    const method = data.split("_")[1];                         user.method = method;
    await user.save();
...
After payment click below:`,
    ...
    await user.save();                                                                                                    bot.sendMessage(chatId,
...
Trust wallet or Binance web3`);
    ...
    const order = await Order.findOne({ orderId });            if (order && order.processed) {
      const targetUser = await User.findOne({ orderId });        const balance = getBalance(targetUser.amount);
      bot.sendMessage(targetUser.chatId, `🎉 Completed\n\n 💰 Balance: ${balance}`);                                        return bot.answerCallbackQuery(query.id, { text: "Already processed" });
    }

    const targetUser = await User.findOne({ orderId });
    if (!targetUser) return;

    await Order.create({ orderId, processed: true });          const balance = getBalance(targetUser.amount);             bot.sendMessage(targetUser.chatId, `🎉 Completed\n\n💰 Balance: ${balance}`);                                     
    ...
    const orderId = Number(data.split("_")[1]);                const order = await Order.findOne({ orderId });
    ...
    bot.answerCallbackQuery(query.id);                       } else if (data.startsWith("reply_")) {                      const userId = data.split("_")[1];
    user.replyTo = userId;                                     user.step = "admin_reply";
    await user.save();

    bot.sendMessage(chatId, "✍️ Send reply message:");
  }
});                                                        
// ---------------- MESSAGE ----------------               bot.on("message", async (msg) => {
  const chatId = msg.chat.id;                                const text = msg.text;
                                                             const username = msg.from.username ? `@${msg.from.username}` : "NoUsername";                                          const name = msg.from.first_name || "User";
                                                             const user = await getUser(chatId, username, name);

  if (user.step === "admin_reply") {
    const targetId = user.replyTo;
    bot.sendMessage(targetId, `📩 Admin Reply:\n\n${text}`);                                                              bot.sendMessage(chatId, "✅ Reply sent");
                                                               user.step = null;                                          await user.save();                                         return;                                                  }
...
${text}`,                                                        {                                                            reply_markup: {
  ...
  if (user.step === "details") {
    ...
    bot.sendMessage(chatId, "💰 Enter amount sent:");          return;                                                  }                                                        
  if (user.step === "amount") {                                const amount = parseInt(text);
    if (isNaN(amount) || amount <= 0) return bot.sendMessage(chatId, "❌ Please enter a valid number");
                                                               if (user.isDemo && amount < 2) return bot.sendMessage(chatId, "❌ Minimum $2 for demo");                              if (!user.isDemo && amount < 20) return bot.sendMessage(chatId, "❌ Minimum $20");

    user.amount = amount;
    // ✅ Unique orderId for concurrent users
...
Thanks for your patience 🙌`);
                                                               bot.sendMessage(ADMIN_ID,                              `New Order                                                 User: ${chatId} (${name} ${username})                      Amount: $${amount}
Method: ${user.method}

${user.details}`,
      {
        reply_markup: {
          inline_keyboard: [                                           [{ text: "Approve", callback_data: `approve_${user.orderId}` },
...
bot.onText(/\/broadcast (.+)/, async (msg, match) => {
  ...
  const text = match[1];                                     const usersList = await User.find();
  for (let u of usersList) {                                   bot.sendMessage(u.chatId, `📢 ${text}`).catch(() => {});
  }
